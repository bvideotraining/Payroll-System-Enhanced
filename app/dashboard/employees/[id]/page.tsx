...
export default function EditEmployeePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    if (orgLoading || fetchedRef.current) return;
      ...
      try {
        ...
        if (emp) {
          setFormData({
            ...
            category: emp.category || EMPLOYEE_CATEGORIES[0],
            department: emp.department || (departments.length > 0 ? departments[0].name : ''),
            branch: emp.branch || (branches.length > 0 ? branches[0].name : 'Royal'),
            jobTitle: emp.jobTitle || emp.role || (jobTitles.length > 0 ? jobTitles[0].title : ''),
            salary: emp.salary?.toString() || '',
            startDate: emp.startDate || emp.joinDate || '',
    ...
    fetchEmployee();
  }, [id, orgLoading, departments, branches, jobTitles, getEmployee]);
            ...
            <div className="grid grid-cols-1 gap-x-4 gap-y-4 sm:grid-cols-2">
              <div>
                ...
                <select
                  id="category"
                  name="category"
                  className="mt-1 flex h-8 w-full rounded-md border border-input bg-background px-2 py-1 text-[11px] ring-offset-background file:border-0 file:bg-transparent file:text-[11px] file:font-medium placeholder:text-muted-foreground focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ring focus-visible:ring-offset-1 disabled:cursor-not-allowed disabled:opacity-50"
            ...
            <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 space-y-3">
              <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-end">
                <div className="sm:col-span-3">
                  ...
                  <Input id="docType" name="type" placeholder="e.g. Contract" value={docForm.type} onChange={handleDocChange} className="mt-1 h-7 text-[10px]" />
                </div>
                <div className="sm:col-span-3">
                  <Label htmlFor="docReceived" className="text-[10px]">Received Date</Label>
                  <Input id="docReceived" name="receivedDate" type="date" value={docForm.receivedDate} onChange={handleDocChange} className="mt-1 h-7 text-[10px]" />
                </div>
                <div className="sm:col-span-3">
                  <Label htmlFor="docExpiry" className="text-[10px]">Expiry Date</Label>
                  <Input id="docExpiry" name="expiryDate" type="date" value={docForm.expiryDate} onChange={handleDocChange} className="mt-1 h-7 text-[10px]" />
                </div>
                <div className="sm:col-span-3">
                  <Label htmlFor="docFile" className="text-[10px]">Attachment</Label>
                  <Input id="docFile" type="file" onChange={handleDocFileChange} className="mt-1 h-7 text-[10px] file:h-7 file:py-0" />
                </div>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-12 gap-3 items-end">
                <div className="sm:col-span-9">
                  <Label htmlFor="docNotes" className="text-[10px]">Notes (Optional)</Label>
                  <Input id="docNotes" name="notes" placeholder="Notes..." value={docForm.notes} onChange={handleDocChange} className="mt-1 h-7 text-[10px]" />
                </div>
                <div className="sm:col-span-3">
                  <Button type="button" onClick={addDocument} className="w-full bg-slate-800 hover:bg-slate-900 h-7 text-[10px]">
                    Add Document
                  </Button>
                </div>
              </div>
